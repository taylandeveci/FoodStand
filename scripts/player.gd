...
signal stamina_changed(current_stamina: float, max_stamina: float) # Yeni stamina sinyali
signal down_started
signal recovered
...
# --- STAMINA SİSTEMİ DEĞİŞKENLERİ ---
@export var max_stamina: float = 100.0
@export var stamina_regen_rate: float = 20.0 # Saniyede yenilenecek stamina miktarı
@export var dash_stamina_cost: float = 25.0  # Dash atınca azalacak miktar
@export var attack_stamina_cost: float = 15.0 # Vurduğu zaman azalacak miktar
...
# UI Bar bağlantısı (Sahnendeki ProgressBar düğümünün ismiyle eşleşmeli)
@onready var stamina_bar: TextureProgressBar = $HUD/HUDRoot/StaminaBar

func _ready() -> void:
	add_to_group("player")
	current_health = max_health
	current_stamina = max_stamina # Stamina başlangıçta tam dolu
...
func _process(delta: float) -> void:
	...
	_regenerate_stamina(delta) # Staminanın zamanla dolması
...
# --- YENİ: STAMINA YENİLENME MANTIĞI ---
func _regenerate_stamina(delta: float) -> void:
	if not is_dashing and not is_down and not is_hurt and current_stamina < max_stamina:
		current_stamina = min(current_stamina + stamina_regen_rate * delta, max_stamina)
		_update_stamina_ui()

# --- YENİ: UI BAR GÜNCELLEME SİNYALİ ---
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("attack") and not is_attacking:
		# Sadece yeterli stamina varsa saldırı başlasın
...
func try_dash(direction: float) -> void:
	...
	# YENİ: Yeterli stamina yoksa dash atamaz
	...
	# YENİ: Dash atınca staminayı azalt ve UI güncelle
...
func apply_hot_water_splash() -> void:
	...
	var hit_any_enemy: bool = false # Combat feedback için kontrol
	...
	# YENİ: Skill bir düşmana isabet ederse güçlü hasar feedback'i ver (Hitstop)
	if hit_any_enemy:
...
func start_attack() -> void:
	...
	# YENİ: Saldırı gerçekleştiğinde staminayı azalt
...
func hit_nearest_enemy() -> void:
	...
	if nearest_enemy != null:
		...
		# YENİ: Normal vuruş düşmana değerse hissedilebilir anlık hitstop tetikle
		trigger_hitstop(0.1)
...
func take_damage(amount: int) -> void:
	...
	# YENİ: Oyuncu hasar aldığında hasar feedback'i hissettirmek için daha uzun hitstop
	trigger_hitstop(0.2)
...
# --- YENİ COMBAT JUICE: HİTSTOP (ZAMANI ANLIK DONDURMA/YAVAŞLATMA) ---
func trigger_hitstop(duration: float) -> void:
	Engine.time_scale = 0.03 # Dünyayı milisaniyeliğine neredeyse tamamen durdur
	# Godot 4 uyumlu, donma süresini gerçek zaman ölçeğinde bekleyen sistem:
	await get_tree().create_timer(duration * 0.03, true, false, true).timeout
	Engine.time_scale = 1.0 # Zaman akışını normale döndür
